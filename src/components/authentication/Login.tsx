...
export default function Login({ onLogin, onSwitchToRegister }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const errorObj = JSON.parse(err.message.replace("Both APIs failed: ", ""))
    ...
    <div className="w-full max-w-md bg-white rounded-2xl">
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900">
              Your email
              {errors.email && <span className="ml-1 text-xs text-red-600">({errors.email})</span>}
            </label>
            <input
              id="email"
              name="email"
              ...
              } text-gray-900 rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5`}
          ...
          <div>
            <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900">
              Password
              {errors.password && <span className="ml-1 text-xs text-red-600">({errors.password})</span>}
            </label>
            <input
              type="password"
              id="password"
        ...
        <div className="text-sm text-center">
          ...
          <button onClick={onSwitchToRegister} className="font-medium text-purple-600 hover:underline">
            Create an account
          </button>
        </div>
      </div>
