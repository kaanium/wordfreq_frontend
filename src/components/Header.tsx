      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="flex-shrink-0 flex items-center">
            <div className="flex items-center cursor-pointer" onClick={() => handleTabClick("text")}>
              ...
              <span className="text-xl font-bold text-gray-900">WordFreq</span>
            </div>
          </div>
            ...
            <nav className="md:ml-6 md:flex md:space-x-2">
              {navItems.map((item) => (
                <button
                  ...
                  } transition-colors duration-200 ${item.comingSoon ? "opacity-60 cursor-not-allowed" : ""}`}
                >
                  {item.icon}
                  {item.name}
                  {item.comingSoon && (
                  ...
                  {activeTab === item.id && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-600"></div>}
                </button>
              ))}
            </nav>
