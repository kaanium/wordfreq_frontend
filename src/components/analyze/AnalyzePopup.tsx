...
import { addWordFlashcard, addWordsBulkToFlashcard } from "../../services/FlashcardService"
import type { PopupProps } from "../../types"

const Popup: React.FC<PopupProps> = ({ title, items, onClose, initialExistingWords }) => {
  ...
  const handleAddAllNewWords = async () => {
    ...
    try {
      const newWords = items
        .filter((item) => !existingWords.includes(item.word.toLowerCase()) && item.meanings && item.meanings.length > 0)
        .map((item) => item.word)
      ...
      const response = await addWordsBulkToFlashcard(newWords, newMeanings)
      if (response && response.ok) {
        setMessage({
          text: `Added ${newWords.length} new words to flashcards`,
          type: "success",
        })
        setExistingWords((prev) => [...prev, ...newWords.map((w) => w.toLowerCase())])
      } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 p-5 overflow-y-auto bg-gray-50 dark:bg-[#121218]" id="scrollableDiv">
          {paginatedItems.length > 0 ? (
            <ul className="space-y-1">
              {paginatedItems.map((wordObj, index) => {
                const isInDatabase = isWordInDatabase(wordObj.word)
                return (
                  <li
                    key={index}
                    className={`py-4 px-4 hover:bg-white dark:hover:bg-[#1E1E2A] rounded-lg transition-colors duration-150 ${
                      isInDatabase ? "border-l-4 border-purple-400 dark:border-purple-600" : ""
                    }`}
                  >
                    <div className="flex justify-between items-center">
                      <h3
                        className={`text-lg font-semibold ${
                          isInDatabase ? "text-purple-700 dark:text-purple-400" : "text-gray-800 dark:text-[#F8F8FC]"
                        }`}
                      >
                        {wordObj.word}
                        {isInDatabase && (
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {!isInDatabase && (
                          <button
                            onClick={() => handleAddWord(wordObj.word, wordObj.meanings)}
                            disabled={isLoading}
                    ...
                    <div className="mt-3 bg-white dark:bg-[#2A2A3A] p-3 rounded-lg border border-gray-100 dark:border-[#32324A]">
                      ...
                      <ul className="list-disc pl-5 space-y-1">
                        {wordObj.meanings.map((meaning, i) => (
                          <li key={i} className="text-gray-700 dark:text-[#F8F8FC] text-sm">
                            {meaning}
                          </li>
                        ))}
                      </ul>
            ...
            <div className="flex flex-col items-center justify-center py-10 text-gray-500 dark:text-[#A0A0B8] bg-white dark:bg-[#1E1E2A] rounded-lg border border-gray-100 dark:border-[#32324A] p-8">
              ...
              <p className="text-xl font-medium mb-2 dark:text-[#F8F8FC]">No matching words found</p>
        ...
        {filteredItems.length > itemsPerPage && (
          <div className="p-5 border-t border-gray-200 dark:border-[#32324A] flex items-center justify-between bg-white dark:bg-[#1E1E2A]">
            <div className="text-sm text-gray-500 dark:text-[#A0A0B8]">
              Showing {(page - 1) * itemsPerPage + 1} to {Math.min(page * itemsPerPage, filteredItems.length)} of{" "}
